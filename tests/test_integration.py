...
class TestGraphIndexFallback:
    def test_fallback_creates_empty_index_when_from_existing_fails(self):
        ...
        with patch.object(nexus_indexes, "setup_settings"), \
             patch("nexus.indexes.Neo4jPropertyGraphStore", return_value=mock_graph_store), \
             patch("nexus.indexes.PropertyGraphIndex.from_existing",
                   side_effect=Exception("graph not found")), \
             patch("nexus.indexes.PropertyGraphIndex.from_documents",
...
class TestDeleteTenantDataErrorReporting:
    async def test_neo4j_failure_reported_in_return(self):
        with patch.object(neo4j_backend, "delete_data", side_effect=Exception("bolt closed")), \
    ...
    async def test_qdrant_failure_reported_in_return(self):
        with patch.object(neo4j_backend, "delete_data"), \
             patch.object(qdrant_backend, "delete_data", side_effect=Exception("connection timeout")):
            result = await nexus_tools.delete_tenant_data("PROJ")
        assert "Partial failure" in result
        assert "Qdrant" in result

    async def test_both_failure_reports_both_backends(self):
        with patch.object(neo4j_backend, "delete_data", side_effect=Exception("n4j down")), \
             patch.object(qdrant_backend, "delete_data", side_effect=Exception("qdrant down")):
            result = await nexus_tools.delete_tenant_data("PROJ")
        assert "Neo4j" in result
        assert "Qdrant" in result
